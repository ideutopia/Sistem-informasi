...
$records_per_page = 10;
//Blok eksekusi SQL
$stmt = $pdo->prepare("SELECT * FROM keuangan WHERE tipe = 'Pengeluaran' ORDER BY id LIMIT :current_page, :record_per_page");
...
<div class="container">
    <h2 class="mb-3 mt-3">Log Data Pengeluaran Keuangan</h2>
    ...
    if (isset($_GET['bulan'])) {
        $filter = $_GET['bulan'];
        echo "Menampilkan data pengeluaran pada bulan : " . "<b>" . $filter . "</b>";
    }
    ?>
    ...
    <div class="table-responsive mt-3">
        <table class="table table-striped mt-2">
            ...
            <tbody>
                ...
                if (isset($_GET['bulan'])) {
                    $cari = $_GET['bulan'];
                    $stmt = $pdo->prepare("SELECT * FROM keuangan WHERE tipe = 'Pengeluaran' AND bulan = '$cari' ORDER BY log DESC LIMIT :current_page, :record_per_page");
                    $stmt->bindValue(':current_page', ($page - 1) * $records_per_page, PDO::PARAM_INT);
                    $stmt->bindValue(':record_per_page', $records_per_page, PDO::PARAM_INT);
                    $stmt->execute();
                    //fetch seleuruh record
                ...
                <?php foreach ($logKeuangan as $log) : ?>
                    ...
                    <tr>
                        ...
                        <td><?= $log['log'] ?></td>
                        <!-- <td class="actions">
                            <a href="update.php?id=<?= $log['id'] ?>" class="edit"><i class="fas fa-pen fa-sm"></i></a>
                            <a href="delete.php?id=<?= $log['id'] ?>" class="trash"><i class="fas fa-trash fa-sm"></i></a>
                        </td> -->
                    </tr>
